...
var upgrader = websocket.Upgrader{
	CheckOrigin: func(r *http.Request) bool {
		return true //允许任何来源的WebSocket连接，这个主要是为了防止同源策略
	},
}
...
func main() {
	...
	r.GET("/:username", func(c *gin.Context) {
		username := c.Param("username") //从请求的url里解析username出来
		c.HTML(http.StatusOK, "chat.html", gin.H{"username": username})  //把username传到前端去
	})
	//前端来建立websocket连接
...
func handleWebSocket(c *gin.Context) {
	conn, err := upgrader.Upgrade(c.Writer, c.Request, nil)   //获得了websocket.Conn对象conn
	if err != nil {   //如果出错的话
		fmt.Println("Error upgrading to WebSocket:", err)
		return
	}
	defer conn.Close()  //在整个函数执行完之后执行
